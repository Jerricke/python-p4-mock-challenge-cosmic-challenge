...
class Planet(db.Model, SerializerMixin):
    ...
    missions = db.relationship(
        "Mission", cascade="all, delete-orphan", backref="scientists"
    )
...
class Scientist(db.Model, SerializerMixin):
    ...
    # Add relationship
    missions = db.relationship(
        "Mission", cascade="all, delete-orphan", backref="planets"
    )
...
class Mission(db.Model, SerializerMixin):
    ...
    @validates("scientists_id")
    def validate_si(self, key, value):
        if not value:
            raise ValueError("Mission must have scientist id")
        return value

    @validates("planet_id")
    def validate_pi(self, key, value):
        if not value:
